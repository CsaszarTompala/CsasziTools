...
- **Accommodation management** — manual one-by-one adding, date-conflict detection (boundary dates shareable), per-night pricing, trip-range calendar highlighting. Location is entered first; the name auto-fills to "Staying at &lt;location&gt;" — tapping the name field clears it for custom input, leaving it empty restores the auto-generated name. Adding, editing, or deleting an accommodation automatically re-estimates all moving-day driving distances in the background so the fuel cost in Travel Mode is always up-to-date. When adding, the map centres on the trip's starting point; when editing, the map centres on the accommodation's saved coordinates (no re-geocoding needed). Geocoded latitude/longitude are persisted on each accommodation so the map reliably returns to the right location.
...
- **Automatic toll finder (GPT-only, exact-route)** — on "Find tolls", Travel Mode builds the exact chronological drive segments of the trip (same segment set as Fuel Breakdown), then asks OpenAI GPT for tolls/vignettes/ferries strictly for those listed routes only (to avoid unrelated extras). An OpenAI key is required; the Google Routes API is used only for driving time/distance estimation, not for toll finding. Auto-generated toll prices are stored in the trip's selected display currency.
- **Start / End point** — separate start and end points for the trip (both default to current location). At trip creation, the default display currency is also auto-selected from the detected current country (e.g. Budapest → `HUF`); if that country currency is not in the app currency list, it falls back to `EUR`. The first day's drive goes from the start point to the accommodation; the last day's drive goes from accommodation to the end point.
