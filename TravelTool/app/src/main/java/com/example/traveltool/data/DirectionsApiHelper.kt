...
object DirectionsApiHelper {
    ...
    private data class RouteTollEstimate(
        val price: Double,
        val currency: String,
    )

    /**
     * Find tolls for already-determined trip drive segments using Google Routes API.
     * Each returned toll entry corresponds to one driven segment that has toll costs.
     */
    suspend fun findTollsForRouteSegments(
        routeSegments: List<TripDriveSegment>,
        googleApiKey: String,
        emissionType: VehicleEmissionType = VehicleEmissionType.GASOLINE,
    ): List<TollRoad> = withContext(Dispatchers.IO) {
        if (routeSegments.isEmpty() || googleApiKey.isBlank()) {
            return@withContext emptyList()
        }

        val tolls = mutableListOf<TollRoad>()
        for (segment in routeSegments) {
            if (segment.fromLocation.isBlank() || segment.toLocation.isBlank()) continue
            val tollEstimate = computeRouteTollGoogle(
            ...
            if (tollEstimate.price > 0.0) {
                tolls.add(
                    TollRoad(
                        id = UUID.randomUUID().toString(),
                        name = "Day ${segment.dayNumber}: ${segment.fromLabel} → ${segment.toLabel}",
                        price = tollEstimate.price,
                        currency = tollEstimate.currency,
...
If there are no tolls at all, respond with: []"""
    ...
    /**
     * Hybrid toll finder: builds a precise route description from the actual
     * drive segments (same ones used in fuel breakdown), then asks GPT to
     * identify tolls, vignettes, and road fees for those exact routes only.
     *
     * @param routeSegments  the chronological drive segments of the trip
     * @param openAiApiKey   OpenAI API key
     * @param travelMode     CAR / MICROBUS (affects vehicle category in prompt)
     * @param model          OpenAI model id
     */
    suspend fun findTollsForSegmentsViaGpt(
        routeSegments: List<TripDriveSegment>,
        openAiApiKey: String,
        travelMode: TravelMode = TravelMode.CAR,
        model: String = "gpt-4o-mini",
    ): List<TollRoad> = withContext(Dispatchers.IO) {
        if (routeSegments.isEmpty() || openAiApiKey.isBlank()) {
            return@withContext emptyList()
        }

        // Build numbered list of exact drive legs
        val routeDescription = routeSegments.mapIndexed { i, seg ->
            "${i + 1}. Day ${seg.dayNumber}: ${seg.fromLabel} (${seg.fromLocation}) → ${seg.toLabel} (${seg.toLocation})" +
                if (seg.distanceKm > 0) " [~${seg.distanceKm.toInt()} km]" else ""
        }.joinToString("\n")
        ...
        val prompt = """You are an expert on road tolls, vignettes, motorway fees, and ferry crossings worldwide. I am driving $vehicleType. Below are the EXACT drive segments of my trip — analyse ONLY these specific routes:

$routeDescription

For ONLY the routes listed above, identify ALL of the following that apply:
1. Country vignettes / e-vignettes / motorway stickers needed (e.g. Austrian "Vignette", Hungarian "e-Matrica / e-vignette", Czech "e-Dálniční známka", Slovenian "e-Vinjeta", Swiss "Vignette", etc.)
2. Specific per-use toll sections (tunnels, bridges, specific motorway sections) that are on these routes — if a toll is crossed in both directions across different segments, list it TWICE (once per direction)
3. Ferry crossings that are part of these routes
4. Any other mandatory road fees for using motorways/highways on these routes

RULES:
- ONLY include tolls/vignettes for countries that the listed routes actually pass through. Do NOT guess extra countries or routes.
- For vignettes, use the shortest available duration (e.g. 10-day). Each vignette only once even if multiple segments cross that country.
- Per-use tolls crossed in both directions must be listed twice with direction noted.
- Give realistic current prices in EUR.
- For ${if (travelMode == TravelMode.MICROBUS) "a minibus — use the appropriate vehicle category" else "a regular car — use category 1 / passenger car prices"}.
- Do NOT include fuel costs, only road/toll/ferry fees.
- If the routes pass through a country with no motorway tolls/vignettes, do NOT list it.

Respond with ONLY a JSON array. No text before or after. No markdown. Each object must have exactly these fields:
...
If no tolls at all: []"""
        ...
        try {
            ...
            Log.d(TAG, "GPT toll query with ${routeSegments.size} segments")
            Log.d(TAG, "Route:\n$routeDescription")
            ...
            val choices = json.getAsJsonArray("choices")
            if (choices == null || choices.size() == 0) {
                Log.e(TAG, "No choices in GPT response")
                return@withContext emptyList()
            }
            ...
            Log.d(TAG, "GPT toll response: $content")

            val jsonContent = extractJsonArray(content)
            if (jsonContent == null) {
                Log.e(TAG, "Could not extract JSON array from GPT response")
                return@withContext emptyList()
            }
            ...
            Log.d(TAG, "Parsed ${tollResults.size} tolls from GPT")
            ...
            Log.e(TAG, "GPT toll exception: ${e.message}", e)
            emptyList()
        }
    }
...
Example: {"totalDistanceKm":1250,"legs":[{"from":"Budapest","to":"Vienna","distanceKm":243},{"from":"Vienna","to":"Budapest","distanceKm":243}]}"""
    ...
    /**
     * Use Google Routes API toll computation to estimate toll price for one segment.
     */
    private suspend fun computeRouteTollGoogle(
        from: String,
        to: String,
        apiKey: String,
        emissionType: VehicleEmissionType = VehicleEmissionType.GASOLINE,
        ...
        val requestBody = """
        {
            ...
            "routeModifiers": {
                "vehicleInfo": {
                    "emissionType": "${emissionType.name}"
                }
            }
        }
        """.trimIndent()
        ...
        val request = Request.Builder()
            ...
            .header("Content-Type", "application/json")
            .header("X-Goog-Api-Key", apiKey)
            .header("X-Goog-FieldMask", "routes.travelAdvisory.tollInfo")
            .post(requestBody.toRequestBody(JSON_MEDIA))
            .build()

        Log.d(TAG, "Toll query: $from → $to")

        val response = client.newCall(request).execute()
        val body = response.body?.string() ?: ""

        if (!response.isSuccessful) {
            Log.w(TAG, "Google toll API error ${response.code}: $body")
            return null
        }

        Log.d(TAG, "Toll response: $body")

        val json = JsonParser.parseString(body).asJsonObject
        val routes = json.getAsJsonArray("routes")
        if (routes == null || routes.size() == 0) {
            Log.w(TAG, "Toll query: no routes returned")
            return null
        }

        val route = routes[0].asJsonObject
        val advisory = when {
            route.has("travelAdvisory") -> route.getAsJsonObject("travelAdvisory")
            route.has("routeTravelAdvisory") -> route.getAsJsonObject("routeTravelAdvisory")
            else -> null
        }
        if (advisory == null) {
            Log.d(TAG, "Toll query: no travelAdvisory in response")
            return null
        }

        val tollInfo = advisory.getAsJsonObject("tollInfo")
        if (tollInfo == null) {
            Log.d(TAG, "Toll query: no tollInfo in advisory")
            return null
        }
        val estimatedPrices = tollInfo.getAsJsonArray("estimatedPrice")
        if (estimatedPrices == null || estimatedPrices.size() == 0) {
            Log.d(TAG, "Toll query: tollInfo exists but no estimatedPrice")
            return null
        }

        val firstPrice = estimatedPrices[0].asJsonObject
        val units = firstPrice.get("units")?.asString?.toDoubleOrNull() ?: 0.0
        val nanos = firstPrice.get("nanos")?.asDouble ?: 0.0
        val currency = firstPrice.get("currencyCode")?.asString?.ifBlank { "EUR" } ?: "EUR"
        val amount = units + (nanos / 1_000_000_000.0)

        return RouteTollEstimate(
            price = amount,
            currency = currency,
        )
    }
